...
import { setglobalpublicKey } from "../api/state";
...
export default function Authscreen({props,navigation}) {
  ...
  useEffect(() => {
    ...
    if (/onConnect/.test(url.pathname)) {
      ...
      setglobalpublicKey(new PublicKey(connectData.public_key))
  ...
  return (
    <Animated.View entering={SlideInRight} style={{ flex: 1 }}>
      <StatusBar style="light" />
        <Btn color="purple" title="Connect with Phantom" onPress={connect} />
      </Animated.View>
  );
}

const Btn = ({ title, onPress,color }: { title: string; onPress: () => Promise<void>,color: string }) => {
  return (
    <View style={{ marginVertical: 10 }}>
        <FAB color={color} label={title} onPress={onPress} />
    </View>
  );
};
